                                 ...
                                 struct bt_gatt_discover_params *params)
{
	...
	LOG_DBG("Found attribute at handle %u", attr->handle);

	if (params->type == BT_GATT_DISCOVER_CHARACTERISTIC) {
		struct bt_gatt_chrc *chrc = (struct bt_gatt_chrc *)attr->user_data;
		
		if (!bt_uuid_cmp(chrc->uuid, BT_UUID_BAS_BATTERY_LEVEL)) {
			LOG_INF("Found Battery Level characteristic at handle %u", chrc->value_handle);
			...
			if (err) {
				LOG_ERR("Failed to discover CCC (err %d)", err);
			}
